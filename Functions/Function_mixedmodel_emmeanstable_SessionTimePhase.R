...
mixedmodel_emmeanstable_SessionTimePhase <- function(myemmeans, mymodel) { 
  ...
  EStable <- as.data.frame(ES) %>% dplyr::select(contrast, SessionTime, Phase, effect.size)
  
  # Extract contrasts# Extract contraeffectsize()sts
  emmeanstable <- as.data.frame(myemmeans$contrasts) %>% 
    left_join(EStable, by=c('contrast','SessionTime','Phase')) %>%
