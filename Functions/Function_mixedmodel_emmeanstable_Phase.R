...
mixedmodel_emmeanstable_Phase <- function(myemmeans, mymodel) { 
  ...
  EStable <- as.data.frame(ES) %>% dplyr::select(contrast, Phase, effect.size)
  
  # Extract contrasts# Extract contraeffectsize()sts
  emmeanstable <- as.data.frame(myemmeans$contrasts) %>% 
    left_join(EStable, by=c('contrast','Phase')) %>%
