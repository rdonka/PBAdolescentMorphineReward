...
mixedmodel_emmeanstable_Session <- function(myemmeans, mymodel) { 
  ...
  EStable <- as.data.frame(ES) %>% dplyr::select(contrast, Session_collapsed, effect.size)
  
  # Extract contrasts# Extract contraeffectsize()sts
  emmeanstable <- as.data.frame(myemmeans$contrasts) %>% 
    left_join(EStable, by=c('contrast','Session_collapsed')) %>%
